...
const CasiShow = () =>{
    ...
    const {Casi} = route.params;
    const[songs,setSongs] = useState([]);

    useEffect(() => {
        const fetchData = async () => {
          try {
            const response = await axios.get(`http://thamlam0011-001-site1.anytempurl.com/api/Song/SearchSinger?tenCasi=${Casi}`);
        ...
        <SafeAreaView style= {styles.SafeAreaView}>
            ...
            <View style= {styles.body}>
                <View style= {styles.body_playlist}>
                    <Text style= {styles.mv_header}>{Casi}</Text>
